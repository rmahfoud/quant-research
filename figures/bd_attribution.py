...
def waterfall(ax, parts: dict[str, float], title: str) -> None:
    ...
    for i, (lab, v) in enumerate(zip(labels, vals)):
        ...
        ax.text(
            i, running[i] + v + off, f"{v:+.1f}", ha="center", va="center", fontsize=8.5, color=INK, fontweight="bold"
        )
        if i < len(labels) - 1:
    ...
    ax.text(
        n,
        parts["_total"] + (0.9 if parts["_total"] >= 0 else -0.9),
        f"{parts['_total']:+.1f}",
        ha="center",
        va="center",
        fontsize=9.5,
...
def main() -> None:
    ...
    for label, mat in (("2-year", 2.0), ("5-year", 5.0), ("10-year", 10.0), ("30-year", 30.0)):
        dd, _ = risk(Y10, mat)
        print(f"  {label:>8}: duration {dd:5.2f}, one-year breakeven on 4% carry alone = {0.04 / dd * 1e4:5.0f}bp")
